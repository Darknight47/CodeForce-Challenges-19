...
----------------------------------------------------- Link for the challenge: https://codeforces.com/contest/2183/problem/A -----------------------------------------------------
...
In a player's turn, he or she chooses two integers l and r such that 1≤l<r≤|a| (here, |a| denotes the current length of a). 
Then, the subarray [al,al+1,…,ar] is replaced with a single number 1−min(al,al+1,…,ar). 
That is, if all numbers in the subarray are 1, then the subarray [al,al+1,…,ar] is removed, and the number 0 is inserted where the subarray was. 
Otherwise, the subarray [al,al+1,…,ar] is removed, and the number 1 is inserted where the subarray was.
...
The first line of each test case contains a positive integer n (3 ≤ n ≤ 100), denoting the length of the array a.
