...
---------------------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1526/A ----------------------------------------------

You are given an array a of 2n distinct integers. 
...
for _ in range(cases):
    ...
    small_half = arr[:sze // 2]
    big_half = arr[sze // 2:]
    result = [0] * sze
    for i in range(sze):
        if i % 2 == 0:
            result[i] = small_half[i // 2]
        else:
            result[i] = big_half[i // 2]
    print(*result)