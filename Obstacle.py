...
---------------------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1547/A --------------------------------------
...
Each test case contains three lines. The first one contains two integers xA,yA (1 ≤ xA, yA ≤ 1000) — coordinates of the start cell A. 
...
for _ in range(cases):
    ...
    tempMin_x = min(xA, xB)
    tempMax_x = max(xA, xB)
    ans = (tempMax_y - tempMin_y) + (tempMax_x - tempMin_x)
    if(xA == xB and xB == xF):
        if(tempMin_y <= yF and yF <= tempMax_y):
            ans += 2
    if(yA == yB and yB == yF):
        if(tempMin_x <= xF and xF <= tempMax_x):
            ans += 2
    print(ans)